...
class Spreadsheet_Excel_Reader {
	...
	function val($row, $col, $sheet = 0) {
		$col = $this->getCol($col);
		// Pastikan $sheet, $row, dan $col ada di dalam array
		if (isset($this->sheets[$sheet]['cells'][$row][$col])) {
			return $this->sheets[$sheet]['cells'][$row][$col];
		}
		return ""; // Kembalikan nilai kosong jika data tidak ditemukan
	}
	...
	function info($row, $col, $type = '', $sheet = 0) {
		$col = $this->getCol($col);
		if (
			isset($this->sheets[$sheet]['cellsInfo'][$row][$col][$type])
		) {
	...
	function rowcount($sheet = 0) {
		return isset($this->sheets[$sheet]['numRows']) ? $this->sheets[$sheet]['numRows'] : 0;
	}
	
	function colcount($sheet = 0) {
		return isset($this->sheets[$sheet]['numCols']) ? $this->sheets[$sheet]['numCols'] : 0;
	}
